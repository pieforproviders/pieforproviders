...
class TimeConversionEngine
  ...
  def call
    ...
    return { full_time: 0, part_time: 0 } if time_rule.blank?
    ...
    { full_time: 1, part_time: 1 } if time_rule.name == 'Full - Partial Day Nebraska'
  end
  ...
  def find_state_rule(service_day, state)
    ...
    state_rules = StateTimeRule.where(state:)
