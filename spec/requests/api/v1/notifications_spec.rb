...
RSpec.describe 'Api::V1::Notifications' do
  ...
  let!(:approval) { create(:approval, num_children: 1, business:, expires_on: 3.days.after) }
  let!(:early_approval) { create(:approval, num_children: 1, business:, expires_on: 1.day.after) }
  let!(:late_approval) { create(:approval, num_children: 1, business:, expires_on: 29.days.after) }
  ...
  describe 'GET /api/v1/notifications/::id' do
    ...
    context 'when logged in as an admin user' do
      ...
      it 'returns not found when notification does not exist' do
        ...
        notify.destroy
        get("/api/v1/notifications/#{notify.id}", params: {}, headers:)
    ...
    context 'when logged in as a non-admin' do
      ...
      it 'returns not found when notification is within the scope of the user' do
        get("/api/v1/notifications/#{non_owner_child.notifications.first.id}", params: {}, headers:)
        expect(response).to have_http_status(:not_found)
      end
    end
  end
  ...
  describe 'POST /api/v1/notifications' do
    ...
    context 'when logged in as an admin user' do
      ...
      it 'returns an error if the child does not exist' do
        existing_approval = create(:approval, num_children: 1, business:, expires_on: 1.day.after)
        child = existing_approval.children.first
        child.destroy
        params = { notification: { child_id: child.id, approval_id: existing_approval.id } }
        post('/api/v1/notifications', params:, headers:)
      ...
      it 'returns an error if the approval does not belong to child' do
        approval_one = create(:approval, num_children: 1, business:, expires_on: 1.day.after)
        approval_two = create(:approval, num_children: 1, business:, expires_on: 1.day.after)
  ...
  describe 'DELETE /api/v1/notifications' do
    ...
    context 'when logged in as an admin user' do
      ...
      it 'returns error if notification does not exist' do
        ...
        delete("/api/v1/notifications/#{does_not_exist.id}", params: {}, headers:)
        expect(response).to have_http_status(:not_found)
      end
    end
    ...
    context 'when logged in as a non-admin' do
      ...
      it 'returns error if notification is not within scope of user' do
        delete("/api/v1/notifications/#{non_owner_child.notifications.first.id}", params: {}, headers:)
        expect(response).to have_http_status(:not_found)
      end
  ...
  describe 'PUT /api/v1/notifications' do
    ...
    context 'when logged in as an admin user' do
      ...
      it 'returns error if notifictaion does not exist' do
        does_not_exist = children.first.notifications.first
        does_not_exist.destroy
        put("/api/v1/notifications/#{does_not_exist.id}", params: {}, headers:)
        expect(response).to have_http_status(:not_found)
      end
    end
    ...
    context 'when logged in as a non-admin' do
      ...
      it 'returns error if notification is not within scope of user' do
        put("/api/v1/notifications/#{non_owner_child.notifications.first.id}", params: {}, headers:)
        expect(response).to have_http_status(:not_found)
      end
