...
RSpec.describe Nebraska::TagsCalculator, type: :service do
  ...
  describe '#tag_hourly_amount' do
    it 'returns correct hourly amount if decimal' do
      ...
      service_day.reload
      expect(described_class.new(service_day:).send(:tag_hourly_amount)).to eq('5.5')
    end
    ...
    it 'returns correct hourly amount if integer' do
      ...
      perform_enqueued_jobs
      service_day.reload
      expect(described_class.new(service_day:).send(:tag_hourly_amount)).to eq('1')
    end
  end
  ...
  describe '#tag_daily_amount' do
    it 'returns correct daily amount' do
      ...
      create(:nebraska_daily_attendance, service_day:)
      perform_enqueued_jobs
      service_day.reload
      expect(described_class.new(service_day:).send(:tag_daily_amount)).to eq('1')
    end
  end
end
