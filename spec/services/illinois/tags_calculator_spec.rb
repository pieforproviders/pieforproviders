...
RSpec.describe Illinois::TagsCalculator, type: :service do
  ...
  describe '#tag_partial' do
    it 'returns correct part tag if time in care is less than 5 hours' do
      ...
      service_day.reload
      expect(described_class.new(service_day:).send(:partial?)).to be(true)
      expect(described_class.new(service_day:).send(:full_day?)).to be(false)
      expect(described_class.new(service_day:).send(:tag_partial)).to eq('1 partDay')
    end
  end
  ...
  describe '#tag_full' do
    it 'returns correct 1 full day if time in care is 5 hours' do
      ...
      perform_enqueued_jobs
      service_day.reload
      expect(described_class.new(service_day:).send(:partial?)).to be(false)
      expect(described_class.new(service_day:).send(:full_day?)).to be(true)
      expect(described_class.new(service_day:).send(:tag_full)).to eq('1 daily')
    end
  ...
  describe '#tag_two_days' do
    it 'returns 2 full days tag if time in care is 17 hours' do
      ...
      expect(described_class.new(service_day:).send(:partial?)).to be(false)
      expect(described_class.new(service_day:).send(:full_day?)).to be(false)
      expect(described_class.new(service_day:).send(:tag_partial)).to be_nil
      expect(described_class.new(service_day:).send(:tag_two_days)).to eq('2 fullDays')
    end
  end
  ...
  describe '#full_and_part_day' do
    it 'returns 1 full day and 1 part day if time in care is 12 hours' do
      ...
      expect(described_class.new(service_day:).send(:tag_partial)).to eq('1 partDay')
      expect(described_class.new(service_day:).send(:tag_full)).to eq('1 daily')
      expect(described_class.new(service_day:).send(:tag_two_days)).to be_nil
    end
