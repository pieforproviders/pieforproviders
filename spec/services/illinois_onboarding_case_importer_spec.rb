...
RSpec.describe IllinoisOnboardingCaseImporter do
  ...
  describe '#call' do
      ...
      it 'creates case records for the correct child with the correct data' do
          ...
          expect(thomas.child_approvals.first.illinois_approval_amounts.count).to eq(1)
          expect(thomas.child_approvals.first.illinois_approval_amounts.first).to have_attributes(
          {
              effective_on: Date.parse('2020-09-01'),
              expires_on: Date.parse('2021-08-31'),
              family_fee: 0
          ...
          expect(becky.child_approvals.first.illinois_approval_amounts.count).to eq(2)
          expect(becky.child_approvals.first.illinois_approval_amounts.find_by(
                  effective_on: Date.parse('2020-11-24')
                  )).to have_attributes(
                  {
                      effective_on: Date.parse('2020-11-24'),
      ...
      it 'updates the existing details if the approval dates are the same but other details are different' do
          ...
          expect(child.illinois_approval_amounts.first.family_fee).to eq(0)
      end
      ...
      it 'updates the child with new approval details if the new approval overlaps' do
          ...
          expect(
          child.illinois_approval_amounts.reject do |naa|
              naa == existing_illinois_approval_amount
          end.first.expires_on
          ).to eq(Date.parse('2021-08-31'))
          expect(existing_illinois_approval_amount.reload.expires_on).to eq(Date.parse('2020-08-31'))
      end
      end
      ...
      fit 'creates case records for every row in the file, idempotently' do
          expect { described_class.new.call }
          .to change(Child, :count)
          .from(0).to(5)
          .and change(Business, :count)
      ...
      it 'updates the child with new approval details if the new approval overlaps' do
          ...
          expect(child.illinois_approval_amounts.reject do |naa|
          naa == illinois_approval_amount
          end.first.expires_on).to eq(Date.parse('2021-08-31'))
          expect(illinois_approval_amount.reload.expires_on).to eq(Date.parse('2020-08-31'))
      end
      end
      ...
      it 'creates case records for the correct child with the correct data' do
          ...
          expect(first_approval.child_approvals.first.illinois_approval_amounts.length).to eq(1)
          expect(
          first_approval.child_approvals.first.illinois_approval_amounts.first.effective_on
          ).to eq(Date.parse('2020-09-01'))
          expect(
          first_approval.child_approvals.first.illinois_approval_amounts.first.expires_on
          ).to eq(Date.parse('2021-08-31'))
          expect(
          first_approval.child_approvals.first.illinois_approval_amounts.first.family_fee
          ).to eq(Money.from_amount(0))
          expect(second_approval.child_approvals.first.illinois_approval_amounts.length).to eq(1)
          expect(
          second_approval.child_approvals.first.illinois_approval_amounts.first.effective_on
          ).to eq(Date.parse('2021-09-01'))
          expect(
          second_approval.child_approvals.first.illinois_approval_amounts.first.expires_on
          ).to eq(Date.parse('2022-08-31'))
          expect(
          second_approval.child_approvals.first.illinois_approval_amounts.first.family_fee
      ...
      it 'updates the existing details if approval periods are different' do
          ...
          expect(child.illinois_approval_amounts.order(:effective_on).first.effective_on)
          .to eq(Date.parse('2020-11-24'))
          expect(child.illinois_approval_amounts.order(:effective_on).last.effective_on)
          .to eq(Date.parse('2021-05-24'))
          expect(child.illinois_approval_amounts.order(:effective_on).first.expires_on)
          .to eq(Date.parse('2021-05-23'))
          expect(child.illinois_approval_amounts.order(:effective_on).last.expires_on)
          .to eq(Date.parse('2021-11-23'))
          expect(child.illinois_approval_amounts.order(:effective_on).first.family_fee)
          .to eq(Money.from_amount(60))
          expect(child.illinois_approval_amounts.order(:effective_on).last.family_fee)
