...
namespace :zipcodes do
  ...
  task :update_states => :environment do
    ...
    CSV.foreach(file.path, :headers => true) do |row|
      ...
      state = Lookups::State.where(abbr: row['abbr']).first_or_initialize
  ...
  task :update_counties => :update_states do
    ...
    CSV.foreach(file.path, :headers => true) do |row|
      ...
      # lookup state
      state = Lookups::State.find_by_abbr!(row['state'])
      county = Lookups::County.where(name: row['name'], state_id: state.to_param).first_or_initialize
  ...
  task :update_zipcodes => :update_counties do
    ...
    CSV.foreach(file.path, :headers => true) do |row|
      ...
      state = Lookups::State.find_by_abbr!(row['state'])
      begin
        county = Lookups::County.find_by_name_and_state_id!(row['county'], state.to_param)
      rescue Exception => e
        puts ">>> e: [#{e}]"
        puts ">>>> No county found for zipcode: [#{row['code']}], '#{row['city']}, #{row['state']}, #{row['county']}... SKIPPING..."
        next
      end
      zipcode = Lookups::Zipcode.where(code: row['code']).first_or_initialize
