...
task update_rate_expiry_date2021: :environment do
  NebraskaRate.where(effective_on: '2021-07-01').find_each do |rate|
    rate.update!(expires_on: '2022-06-30'.to_date.end_of_day)
  end

  ServiceDay.where(date: ('2022-07-01'.to_date)..Time.current.at_end_of_day).find_each do |service_day|
